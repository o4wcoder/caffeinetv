...
fun formatFriendsWatchingString(context: Context, broadcaster: Lobby.Broadcaster): String? {
    ...
    return when (broadcaster.followingViewersCount) {
        0 -> null
        1 -> context.getString(singleFriendWatchingResId, broadcaster.followingViewers[0].username, broadcaster.followingViewers[0].avatarImageUrl)
        else -> context.resources.getQuantityString(multipleFriendsWatchingResId, broadcaster.followingViewersCount - 1, broadcaster.followingViewers[0].username, broadcaster.followingViewers[0].avatarImageUrl, broadcaster.followingViewersCount - 1)
    }
}
